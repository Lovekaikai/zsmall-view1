		...
		<view class="search-keyword" @touchstart="blur">
			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					...
					<view class="keyword">
						<view v-for="(key,index) in oldKeywordList" @tap="doSearch(key)" :key="index">{{key}}</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			doSearch(key) {
				...
				// });
				uni.navigateTo({
					url: `/pages/list/list?key=${seakey}`
				})
...
<style>
	.search-box {width:95%;background-color:rgb(242,242,242);padding:7.5px 2.5%;}
