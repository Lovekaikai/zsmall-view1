	...
	<view class="alteration">
		<view style="background:#fff;margin-top:16upx;">
			...
			<!-- 	<view class="uploadbox">
					...
					<view class="content">
						<textarea placeholder="请输入问题描述" v-model="description" />
						</view>
				...
				<view class="flex box">
					<view class="kind">换货地址</view>
					<view class="content" @tap="jump" style="font-size: 12px;">
						<view class="" v-if="!lotusAddressData.provinceName">
							{{addresss || '请选择地址'}}
						</view>
						<view class="" v-else>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			let dataStroage = this.$common.getStorageSync("address2")
			if (dataStroage) {
				this.dataAddress = dataStroage
				console.log('dataStroage',dataStroage)
				this.barterSite = `${dataStroage.countryId}${dataStroage.provinceId}${dataStroage.cityId}${dataStroage.districtId}`
				this.barterSiteDetail = `${dataStroage.address}`
				// this.babarterSite=`${dataStroage.countryId}${dataStroage.provinceId}${dataStroage.cityId}`
				this.addresss = `${dataStroage.countryId}${dataStroage.provinceId}${dataStroage.cityId}${dataStroage.districtId}${dataStroage.address}`
			} else {
				this.dataAddress = ''
			}
		},
		methods: {
			jump() {
				uni.navigateTo({
					url: '../addressManage/addressManage?page=exchange'
				})
			},
			radioChange (evt) {
				console.log(evt)
			...
			returnsGoods () {
				var param = {
					...
					barterSite: this.barterSite,
					barterSiteDetail: this.barterSiteDetail,
					goods: [],
				...
				} else if(!this.barterName) {
					this.$common.showToast('联系人不能为空', 'none')
				} else if (!this.barterPhone) {
					this.$common.showToast('联系人电话不能为空', 'none')
				} else if (!this.$common.phoneReg(this.barterPhone)) {
					this.$common.showToast('联系人电话格式错误', 'none')
				} else if (!this.barterSite) {
					this.$common.showToast('地址没有选择', 'none')
				} else {
